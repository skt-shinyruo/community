...
- traceId：`common/src/main/java/com/nowcoder/community/platform/web/reactive/TraceIdWebFilter.java`、`gateway/src/main/java/com/nowcoder/community/gateway/filter/TraceIdSupport.java`（单一注入点）
- 安全异常响应：`common/src/main/java/com/nowcoder/community/platform/web/reactive/ReactiveSecurityExceptionHandler.java`
...
- 可信代理配置：`common/src/main/java/com/nowcoder/community/platform/net/TrustedProxyProperties.java`（`gateway.trusted-proxy.*`；由 common 自动装配）
...
- 若启用统计采集（`analytics.collect.enabled=true`），gateway 会通过 `analytics-api` 的 Dubbo RPC 调用 analytics-service（best-effort）。
  ...
  - Runbook：`.helloagents/modules/runbooks/gateway-analytics-collect.md`
...
- traceId 注入仅由 `TraceIdWebFilter` 负责（由 common 自动装配），避免 WebFilter/GlobalFilter 重复导致的维护成本与潜在覆盖困惑。
