...
（详见 `.helloagents/data.md` 的 “Elasticsearch 索引” 小节）
...
- 2026-02-03：reindex single-flight 增加锁续租（owner=jobId + 原子 renew）避免长任务锁过期并发重建；legacy `/api/search/internal/reindex` 固定返回 410 并提示迁移，降低误用与攻击面。
