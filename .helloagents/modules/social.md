...
- 写路径契约可信：服务端通过 Dubbo RPC 回源 `content-service`（`ContentEntityRpcService#resolveEntity`）解析 entity 的 owner/postId/存在性；content-service 不可用时 fail-closed（返回 503），禁止信任客户端注入字段
...
- 提供 internal RPC：
  - `SocialBlockRpcService`：点查 A/B 是否存在任意方向拉黑（content/message 写路径使用）
...
- Dubbo RPC（服务间同步调用，推荐）：
  ...
  - `SocialBlockRpcService`：拉黑关系点查（content/message 写路径使用；需显式 timeout + retries=0）
...
## Dependencies
- user（用户资料用于列表展示）
- content（写路径 Dubbo RPC 回源解析内容实体：`ContentEntityRpcService#resolveEntity`）
- message（点赞/关注通知）
- infra（Redis/Kafka）

## Change History
- 2026-02-24：新增 `SocialBlockScanRpcService`（拉黑关系扫描）供下游服务投影自举；下游写路径拉黑校验收敛为“本地投影 + scan 自举”，降低同步依赖放大风险。
- 2026-02-25：按需求移除本地投影：删除 content entity 投影与消费者，写路径改为 Dubbo RPC 回源 `content-service` 解析 entity 元信息。
- 2026-02-25：按需求移除遗留 scan RPC：删除 `SocialBlockScanRpcService`/`SocialLikeScanRpcService` 及 DTO/实现（当前无调用点）。
