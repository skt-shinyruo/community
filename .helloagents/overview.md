...
> 本文件包含项目级核心信息。详细模块说明见 `.helloagents/modules/`。
...
> 说明：历史单体（迁移期模块）已从仓库主干移除；历史决策/迁移记录保留在 `.helloagents/archive/`，用于追溯与对照。
...
> 注：生产级交付相关操作入口见：`deploy/`、`scripts/`、`.helloagents/modules/runbooks/`。
...
- [API 手册](api.md)
- [数据模型](data.md)
- [运行手册：docker compose 启动与回归](modules/runbooks/legacy-cutover.md)
- [变更历史](archive/_index.md)
