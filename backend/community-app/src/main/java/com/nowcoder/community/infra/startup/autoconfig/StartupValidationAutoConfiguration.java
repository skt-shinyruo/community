...
/**
 * prod / production profile 或 production deployment environment 下启用启动期校验（fail-closed）。
 *
 * <p>说明：该 auto-config 不依赖 servlet/reactive 细节，可在所有服务与单体中生效。</p>
 */
@AutoConfiguration
@Conditional(StartupValidationAutoConfiguration.ProductionEnvironmentCondition.class)
public class StartupValidationAutoConfiguration {
    ...
    static final class ProductionEnvironmentCondition implements Condition {

        @Override
        public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
            return context != null
                    && ProductionEnvironmentPredicate.isProduction(context.getEnvironment());
        }
    }
}
