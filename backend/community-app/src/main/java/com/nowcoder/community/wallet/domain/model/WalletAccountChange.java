...
public record WalletAccountChange(
    ...
    public WalletAccountChange {
        ...
        if (nextBalance < 0L) {
            throw new IllegalArgumentException("nextBalance must not be negative");
        }
        ...
        if (previousBalance < 0L) {
            throw new IllegalArgumentException("delta must originate from a non-negative balance");
        }
