...
class PostMediaReferenceSchedulerTest {
    ...
    void deletionSchedulingMustJoinTheExistingPostDeletionTransaction() throws Exception {
        Transactional transactional = PostMediaReferenceScheduler.class
                .getDeclaredMethod("scheduleReleaseForDeletedPost", UUID.class)
                .getAnnotation(Transactional.class);
        if (transactional == null) {
            transactional = PostMediaReferenceScheduler.class
                    .getAnnotation(Transactional.class);
        }
    ...
    void deletedPostShouldScheduleEveryUnfinishedReferenceIncludingBindPendingRace() {
        ...
        PostMediaReferenceScheduler scheduler =
                new PostMediaReferenceScheduler(repository, publisher, CLOCK);

        scheduler.scheduleReleaseForDeletedPost(postId);
    ...
    void schedulingServiceMustNotOwnOssOrPersistenceImplementationCollaborators() {
        assertThat(List.of(PostMediaReferenceScheduler.class.getDeclaredFields()).stream()
                .map(Field::getType)
                .map(Class::getName))
