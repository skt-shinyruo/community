...
class SearchReindexApplicationServiceScanTest {
    ...
    void clearAndReindexShouldUpsertMappedPayloadsAndAdvanceCursorWhenNoTargetIndex() {
        ...
        SearchReindexApplicationService service = new SearchReindexApplicationService(
                repository,
                postScanQueryApi,
                propertiesWithPageSize(2),
                providerOf(null),
                mock(ReindexJobApplicationService.class),
                new SearchReindexDomainService()
        );

        int total = service.clearAndReindexFromContentService();

        ArgumentCaptor<PostSearchDocument> payloadCaptor = ArgumentCaptor.forClass(PostSearchDocument.class);
        verify(repository).clear();
        verify(repository, times(3)).save(payloadCaptor.capture());
        verify(repository, never()).saveToIndex(any(), org.mockito.ArgumentMatchers.anyString());
        ...
        assertThat(payloadCaptor.getAllValues())
                .extracting(
                        PostSearchDocument::postId,
                        PostSearchDocument::userId,
                        PostSearchDocument::categoryId,
                        PostSearchDocument::tags,
                        PostSearchDocument::title,
                        PostSearchDocument::content,
                        PostSearchDocument::type,
                        PostSearchDocument::status,
                        PostSearchDocument::createTime,
                        PostSearchDocument::score
                )
                .containsExactly(
    ...
    void clearAndReindexShouldUpsertMappedPayloadsToTargetIndexAndAdvanceCursor() {
        PostSearchRepository repository = mock(PostSearchRepository.class);
        PostScanQueryApi postScanQueryApi = mock(PostScanQueryApi.class);
        PostSearchIndexRepository indexRepository = mock(PostSearchIndexRepository.class);
        ...
        when(indexRepository.createNewIndex()).thenReturn("community_posts_v20260329000000");
        when(postScanQueryApi.scanPosts(null, 2)).thenReturn(new PostScanView(
                List.of(
        ...
        ArgumentCaptor<PostSearchDocument> payloadCaptor = ArgumentCaptor.forClass(PostSearchDocument.class);
        verify(indexRepository).ensureAliasReady();
        verify(indexRepository).createNewIndex();
        verify(indexRepository).switchAliasTo("community_posts_v20260329000000");
        verify(indexRepository).cleanupOldIndices();
        verifyNoMoreInteractions(indexRepository);

        verify(repository, never()).clear();
        verify(repository, times(2)).saveToIndex(payloadCaptor.capture(), eq("community_posts_v20260329000000"));
    ...
    private static ObjectProvider<PostSearchIndexRepository> providerOf(PostSearchIndexRepository indexRepository) {
        StaticListableBeanFactory beanFactory = new StaticListableBeanFactory();
        if (indexRepository != null) {
            beanFactory.addBean("postSearchIndexRepository", indexRepository);
        }
        return beanFactory.getBeanProvider(PostSearchIndexRepository.class);
    }
