...
import com.nowcoder.community.social.service.BlockApplicationService;
...
class BlockControllerTest {
    ...
    void blockShouldDelegateToBlockApplicationService() {
        ...
        BlockController.BlockRequest request = new BlockController.BlockRequest();
        request.setUserId(uuid(8));

        Result<Void> result = controller.block(authentication(actorId), request);

        assertThat(result.getCode()).isEqualTo(0);
        verify(blockApplicationService).block(actorId, request.getUserId());
    }
