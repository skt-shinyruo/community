...
create index if not exists idx_virtual_inventory_listing_status on virtual_inventory_unit(listing_id, status, inventory_unit_id);
...
create table if not exists virtual_order (
  ...
  create_time timestamp null default current_timestamp,
  update_time timestamp null default current_timestamp on update current_timestamp,
  constraint uk_virtual_order_request unique (request_id)
);

create index if not exists idx_virtual_order_buyer_time on virtual_order(buyer_user_id, create_time);
create index if not exists idx_virtual_order_seller_time on virtual_order(seller_user_id, create_time);
create index if not exists idx_virtual_order_listing_status on virtual_order(listing_id, status);
create index if not exists idx_virtual_order_auto_confirm on virtual_order(status, auto_confirm_at);
...
create index if not exists idx_virtual_delivery_order on virtual_delivery(order_id, delivery_id);
