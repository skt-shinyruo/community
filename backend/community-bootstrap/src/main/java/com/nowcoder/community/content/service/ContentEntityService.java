...
public class ContentEntityService {
    ...
    public ContentEntityService(DiscussPostMapper discussPostMapper, CommentMapper commentMapper) {
    ...
    private ResolvedEntity resolvePost(int postId) {
        DiscussPost post = discussPostMapper.selectDiscussPostById(postId);
        if (post == null || post.getId() <= 0 || post.getStatus() == 2) {
            throw new BusinessException(POST_NOT_FOUND);
        }
        return new ResolvedEntity(post.getUserId(), post.getId());
    }

    private ResolvedEntity resolveComment(int commentId) {
        ...
        return new ResolvedEntity(comment.getUserId(), postId);
    }
