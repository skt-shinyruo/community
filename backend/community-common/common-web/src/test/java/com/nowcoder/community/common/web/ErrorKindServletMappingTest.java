...
class ErrorKindServletMappingTest {

    @ParameterizedTest(name = "{0} -> HTTP {1}")
    @MethodSource("kindMappings")
    void businessErrorsShouldMapKindToStableStatusAndResultBody(String kindName, int expectedStatus) {
        GlobalExceptionHandler handler = new GlobalExceptionHandler();
        ErrorCode errorCode = errorCode(kindName, 99001, "mapped error");

        ResponseEntity<Result<Void>> response = handler.handleBusiness(new BusinessException(errorCode));

        assertThat(response.getStatusCode().value()).isEqualTo(expectedStatus);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().getCode()).isEqualTo(99001);
        assertThat(response.getBody().getMessage()).isEqualTo("mapped error");
        ...
        return (ErrorCode) Proxy.newProxyInstance(
                ...
                (proxy, method, args) -> switch (method.getName()) {
                    ...
                    case "getHttpStatus" -> throw new AssertionError(
                            "Servlet adapter must map ErrorKind and must not ask ErrorCode for HTTP status"
                    );
