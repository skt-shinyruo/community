...
> 约定：本文档中的命令默认从 `backend/` 目录执行（后端工程根目录）。
...
- `Dockerfile.spring-service`：统一构建 Spring Boot 模块镜像（build arg：`MODULE`）。
  - `MODULE` 取值为 Maven `artifactId`（例如 `community-app`），不是目录路径；因此即使模块目录结构调整（如按域分组），compose 中的 `MODULE: xxx` 也无需随之变更。
...
- `docker-compose.legacy-microservices.yml`：历史微服务版本（Nacos/Gateway + 多服务），仅用于参考或迁移窗口。
- `docker-compose.frontend-direct.legacy-microservices.yml`：历史微服务版本入口覆盖（仅用于参考）。
- `docker-compose.mailhog.yml`：MailHog 覆盖（用于本地接收 SMTP 邮件，覆盖 `community-app` 邮件配置）。
- `docker-compose.mailhog.legacy-microservices.yml`：历史微服务版本 MailHog 覆盖（仅用于参考）。
...
## 常用 docker compose 命令速查（从 backend/ 目录执行）
