...
>
> 约定：本文档中的命令与路径默认以 `backend/` 作为工作目录（除非特别说明）。
...
- **模块化**：代码仍以 Maven 多模块组织（`*-api` / `*-service` / `platform/*`），用“编译期边界”控制长期复杂度。
- **legacy 微服务形态**：历史 `gateway + nacos + 多服务` 仍保留在 `gateway/` 与 `deploy/*legacy-microservices*` 里，仅作为迁移窗口参考，不作为默认路径。
...
### 2.2 后端单体入口（`app/community-app/`）
- 唯一 deployable：`community-app`（`mvn -pl :community-app -am package`）
- 组装方式：
...
- 统一对外安全边界：`app/community-app/.../CommunitySecurityConfig`
  - 对外路径稳定：`/api/**`、`/files/**`
...
- `auth-service`：登录/刷新/登出、验证码、注册/激活、找回密码、登录风控（`auth.login-rate-limit`）
- `user/user-service`：用户资料、角色管理（管理员）、头像上传与文件服务（`GET /files/**`）
- `content/content-service`：帖子/评论/回复、审核、举报、内容分数刷新；并作为内容相关事件的 producer
- `social/social-service`：点赞、关注、拉黑；并作为社交事件的 producer
- `message/message-service`：私信、通知；消费事件生成通知
- `search/search-service`：搜索投影（ES）；消费事件更新索引；提供 legacy 路径 `POST /api/search/internal/reindex`（固定返回 410，引导迁移到 ops）
- `analytics/analytics-service`：统计/分析（Redis）
- `ops-service`：运维平面（`/api/ops/**`），承载 reindex/outbox 回放等高成本动作（管理员入口）
...
- **`platform/contracts-core`（跨模块稳定协议）**：`Result<T>`、错误码、业务异常、内部 RPC 契约（例如 `EntityResolveRpcService`、`UserModerationRpcService`）
- **`platform/contracts-event-core`（事件协议）**：统一 event envelope + 校验 + unknown handling（skip/DLQ 可配置）
- **`platform/infra-*`（横切能力交付）**：
...
- **`platform/common`（运行期共享）**：traceId、全局异常映射、审计日志 filter、可信代理/客户端 IP 解析、启动期校验（prod 下 fail-closed）
...
- `deploy/docker-compose.legacy-microservices.yml`：历史微服务版本（仅参考/迁移窗口）。
...
- 审计日志：`platform/common` 的审计 filter 会对非 GET 的 `/api/**` 打印审计日志（前缀类似 `"[audit][service=community-app]"`）
...
## 7. 与代码一致性的检查清单（建议）
- 对外入口与授权矩阵：以 `app/community-app/src/main/java/.../CommunitySecurityConfig.java` 为准
