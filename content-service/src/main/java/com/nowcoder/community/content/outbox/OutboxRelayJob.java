...
public class OutboxRelayJob {
    ...
    public OutboxRelayJob(
        ...
        if (meterRegistry != null) {
            meterRegistry.gauge("content_outbox_backlog", Tags.of("status", "NEW"), newCountGauge);
            meterRegistry.gauge("content_outbox_backlog", Tags.of("status", "RETRY"), retryCountGauge);
            meterRegistry.gauge("content_outbox_backlog", Tags.of("status", "SENDING"), sendingCountGauge);
            meterRegistry.gauge("content_outbox_backlog", Tags.of("status", "FAILED"), failedCountGauge);
        }
    }

    @Scheduled(fixedDelayString = "${content.events.outbox.relay-interval-ms:5000}")
    public void relay() {
    ...
    @Scheduled(fixedDelayString = "${content.events.outbox.cleanup-interval-ms:21600000}")
