...
/**
 * content-service 拉黑关系投影自举（bootstrap）：
 * - 周期性从 social-service 扫描“当前拉黑关系集合”，回填到本地 user_block_projection
 * - 事件流（BlockRelationChanged）负责实时纠偏；scan 负责冷启动与补洞
 */
@Component
@ConditionalOnProperty(name = "content.projection.block-scan.enabled", havingValue = "true", matchIfMissing = true)
public class BlockProjectionBootstrapJob {
    ...
    public BlockProjectionBootstrapJob(
            ...
            @Value("${content.projection.block-scan.single-flight:true}") boolean singleFlightEnabled,
            @Value("${content.projection.block-scan.lock-ttl-seconds:300}") int lockTtlSeconds,
            @Value("${content.projection.block-scan.batch-size:2000}") int batchSize,
            @Value("${content.projection.block-scan.rescan-interval-ms:3600000}") long rescanIntervalMs,
    ...
    @Scheduled(fixedDelayString = "${content.projection.block-scan.interval-ms:2000}")
    public void bootstrapBlockedRelations() {
        ...
        if (singleFlightEnabled && guard != null) {
            lock = guard.tryAcquire("content:projection_bootstrap_blocks", Duration.ofSeconds(lockTtlSeconds));
            if (lock == null) {
                return;
            }
        }
