...
class CommentServiceTest {
    ...
    void addCommentShouldFailClosedWhenUserServiceUnavailable() {
        ...
        when(socialBlockClient.isEitherBlocked(1, 2)).thenThrow(new BusinessException(CommonErrorCode.SERVICE_UNAVAILABLE, "user-service 不可用"));
    ...
    void addCommentShouldPublishEventWhenOk() {
        ...
        doNothing().when(moderationGuard).assertCanSpeak(eq(1));
        when(socialBlockClient.isEitherBlocked(1, 2)).thenReturn(false);
