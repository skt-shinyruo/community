...
docker run -d --name "${container}" \
  ...
  -v "${REPO_ROOT}/deploy/mysql/primary-init/001_create_databases.sh:/docker-entrypoint-initdb.d/001_create_databases.sh:ro" \
  -v "${REPO_ROOT}/deploy/mysql/primary-init/010_current_schema.sql:/docker-entrypoint-initdb.d/010_current_schema.sql:ro" \
  -v "${REPO_ROOT}/deploy/scripts/run-community-migrations.sh:/migration/run-community-migrations.sh:ro" \
  -v "${REPO_ROOT}/deploy/mysql/community-migrations:/migrations:ro" \
...
migration_checksum="$(sha256sum deploy/mysql/community-migrations/V016__reliability_and_query_consistency.sql | awk '{print $1}')"
