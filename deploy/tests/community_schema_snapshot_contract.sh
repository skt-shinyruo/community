...
business_ddl_files="$(rg -l -i '^create[[:space:]]+table' deploy/mysql --glob '*.sql' \
  | grep -Ev '/(nacos|xxl-job|community-migrations)/' || true)"
...
grep -Fq 'grant select, insert, update, delete, create, alter, index' \
  deploy/mysql/primary-init/001_create_databases.sh
grep -Fq 'COMMUNITY_MIGRATION_USERNAME=community_migrator' deploy/.env.single.example
grep -Fq 'COMMUNITY_MIGRATION_USERNAME=community_migrator' deploy/.env.cluster.example
