...
gateway_runtime_configs=(
  ...
  "${REPO_ROOT}/deploy/config/nacos/community-gateway.yaml"
)
...
if grep -Fq 'trusted-proxy:' "${shared_config}"; then
  fail "deploy/config/nacos/community-shared.yaml must not share one owner domain's trusted proxy CIDRs with another"
fi
...
base_compose="${REPO_ROOT}/deploy/compose/base.yml"
single_frontend_compose="${REPO_ROOT}/deploy/compose/runtime/edge/single.yml"
cluster_frontend_compose="${REPO_ROOT}/deploy/compose/runtime/edge/cluster.yml"
single_env="${REPO_ROOT}/deploy/stacks/single/.env.example"
cluster_env="${REPO_ROOT}/deploy/stacks/cluster/.env.example"
