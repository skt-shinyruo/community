...
grep -Fq 'FROM nginx:1.27-alpine AS frontend-runtime' deploy/Dockerfile.frontend
grep -Fq 'COPY --from=frontend-build /workspace/frontend/dist /usr/share/nginx/html' deploy/Dockerfile.frontend
...
grep -Fq 'pid /tmp/nginx.pid;' deploy/frontend/nginx.conf
grep -Fq 'alias /tmp/community-frontend/app-config.js;' deploy/frontend/nginx.conf
grep -Fq '/app-config.js "no-store, max-age=0";' deploy/frontend/nginx.conf
grep -Fq '~^/assets/ "public, max-age=31536000, immutable";' deploy/frontend/nginx.conf
grep -Fq 'server_tokens off;' deploy/frontend/nginx.conf
grep -Fq 'script-src '\''self'\'';' deploy/frontend/nginx.conf
grep -Fq 'X-Content-Type-Options "nosniff"' deploy/frontend/nginx.conf
grep -Fq 'X-Frame-Options "DENY"' deploy/frontend/nginx.conf
grep -Fq 'location ^~ /api/' deploy/frontend/nginx.conf
grep -Fq 'location ^~ /files/' deploy/frontend/nginx.conf
grep -Fq 'location ^~ /ws/im' deploy/frontend/nginx.conf
if rg -n '^user[[:space:]]+' deploy/frontend/nginx.conf; then
  echo 'nginx user directive is invalid when the container already starts as a non-root user' >&2
  exit 1
fi
if rg -n "script-src[^;]*'unsafe-inline'" deploy/frontend/nginx.conf; then
...
FRONTEND_RUNTIME_IM_HTTP_BASE_URL="${im_value}" \
  ./deploy/frontend/render-runtime-config.sh "${runtime_config}"
...
./deploy/deployment.sh config --topology single --scope full \
  --env-file deploy/.env.single.example --no-observability >"${single_config}"
./deploy/deployment.sh config --topology cluster --scope full \
  --env-file deploy/.env.cluster.example --no-observability >"${cluster_config}"
...
for rendered_config in "${single_config}" "${cluster_config}"; do
  ...
  printf '%s\n' "${frontend_service}" | grep -Fq 'GATEWAY_PUBLIC_BASE_URL: http://localhost:12880'
