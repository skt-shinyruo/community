...
6. auth 生成安全随机的 6 位注册验证码并持久化 `auth.registration-code-mail` outbox。重发先原子消费可信 IP、邮箱和 registration identity 配额，再取得 `ReplacementLease` 并写 pending replacement；worker 取得 `DeliveryClaim` 后由 ApplicationService 发送 SMTP，claim 在成功后完成或重新认领再完成，失败由 outbox 重试并保留原 active code。
