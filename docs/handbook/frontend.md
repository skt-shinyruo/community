...
帖子卡点赞带在途守卫：同一帖子的点赞请求未完成前按钮禁用并忽略重复点击，避免乱序响应把中间态写回界面
与点赞缓存。「已隐藏 N 条来自已屏蔽用户的帖子」按跨页累计口径统计被过滤的帖子（按 id 去重，页间位移
不重复计数），整页刷新时清零重计。
...
UiState（错态带重试）。与帖子流同口径，已屏蔽作者的命中在补水前过滤（搜索命中自带 `userId`），
「已隐藏 N 条来自已屏蔽用户的帖子」按跨页累计口径统计（按 postId 去重，页间位移不重复计数），整页刷新或清空搜索时
清零重计。搜索结果来自 ES 投影、最终一致，页面以固定文案说明发帖 / 编辑后结果可能延迟数秒
...
投影上即时过滤标题 / 描述 / 卖家（`marketState.js` 的 `filterMarketListings`），不产生新请求；无匹配时
UiState 空态给出「清除搜索」下一步。商品卡是 8px 扁平列表语言（整卡 RouterLink 链接、19px/650 标题、
弱表面类型 chip），在售 / 已售罄等状态用 UiBadge 变体区分（ACTIVE success、SOLD_OUT warning，来自投影的
`statusVariant`），价格与库存文本直接取自服务端字段投影；分页保持「加载更多」追加（页码 + 去重归并、
...
`market-*` 全局样式已在 pages.css 退役票中删除），`tokens.test.js` 的 MarketOrderDetailView `.input`
基线登记与 `loading-states.test.js` 的 MarketOrderListView 裸加载文本登记随之移除。

`/market/publish`、`/market/my-listings` 与 `/market/my-listings/:listingId/inventory` 卖家路由也已随
波次 9 完成迁移，三个视图只使用 `<style scoped>` 与 variables.css 令牌，不再依赖 `pages.css` 的市场全局
样式（`market-row` / `market-pill` / `market-workflow-section` / `market-textarea` / `market-select` 等
...
UiField + UiSelect / UiTextarea，库存内容收敛到裁剪版 UiTable（内容 / 类型 / 状态列，类型与状态列挂排序
钩子，排序在已加载投影上本地完成，状态按可售 → 锁定 → 售出 → 失效的秩排序），失效库存属于不可恢复的
高风险写操作，先经 UiModalConfirm（danger 变体）复述内容再执行，追加 / 失效成功后刷新列表并以
role=status 播报结果，失败内联 alert。

市场波次 9 收尾已退役 `pages.css` 中的全部市场全局页面样式：管理后台争议页（AdminMarketDisputesView）只做
行为等价的 CSS 搬移，把仍在使用的 `market-page` / `market-admin-list` / `market-admin-row` /
`market-inline-actions` 规则原样迁入自己的 `<style scoped>`（DOM 类名不变，测试选择器与视觉不受影响），
已无引用的 `market-kicker` / `market-summary` / `market-list-shell` / `market-panel` / `market-section-head` /
`market-order-list` / `market-order-row` / `market-form-grid` / `market-field` 等全局块整体删除。至此市场页面
...
表单原语为 `frontend/src/components/ui/UiInput.vue`、`UiTextarea.vue` 和 `UiField.vue`：`UiInput` / `UiTextarea` 提供 `v-model`（含 trim / number 修饰符）、原生属性透传和禁用状态，`UiInput` 另有 size（md / sm）与 variant（outline / ghost）；`UiField` 承载 label 关联、帮助 / 错误文本（`aria-describedby` / `aria-invalid` / `role=alert`）和 `required` / `pattern` / `invalid` 原生校验语义，不引入表单校验库。字段内的 `UiInput` / `UiTextarea` 自动继承关联与校验状态（`UiAutosuggestInput` 直接组合 `UiInput`，同样自动继承）；其他控件使用默认 slot 的 `controlId` / `describedBy` / `invalid` / `required` 手动接线。`.btn`、`.input`、`.card`、`.skeleton`、`.auth-field`、`.field-label`、`.auth-form` 等是原语内部实现细节，视图不得新增使用；波次 10 后非管理视图已清零，管理后台（Analytics / Moderation / UserManagement / WalletAdmin）保留按行为等价迁入各自 scoped 的 `.input` / `.card` / `.tag` 副本，由 `tokens.test.js` 的全量登记守卫锁定（新增、计数上升或登记失效都会失败），后续管理收敛时应删除对应登记行。
...
`frontend/src/components/ui/UiTable.vue` 是波次 9 首次交付的裁剪版表格原语，只承担语义 table、统一样式和
排序钩子三件事：受控 `columns`（`key` / `label` / `sortable` / `align`）+ `rows` + `rowKey` 渲染
`<table>` / `<caption>`（sr-only，可访问名称）/ `th[scope=col]` / `tbody`，单元格默认显示行字段文本，
`cell-<key>` 具名 slot（scope 含 `row` / `column`）承载徽章、按钮等自定义内容；排序钩子为列头原生
button（click 与 Enter / Space 激活）发出 `sort(columnKey)` 事件，排序状态由视图以 `sortKey` /
`sortDirection` 受控持有，`aria-sort`（ascending / descending）只标注当前排序列，方向图标取自 lucide。
它明确不提供虚拟滚动、冻结列或单元格编辑；横向空间不足时外层容器 `overflow-x: auto` 收缩，不撑破页面
...
所有 dialog 的焦点由原生 `<dialog>` 行为承担：`showModal()` 打开后进入 top layer，Tab / Shift+Tab 原生约束在弹窗内，关闭或卸载后焦点原生回到触发控件；UiModal 只在挂载时聚焦 `[data-autofocus]` 标记的控件。同时保留 `role=dialog`、`aria-modal`、可关联标题 / 描述和 Escape 关闭语义。UiModal / UiModalConfirm / ReportModal / EditContentModal 已接入；ModerationView 的处置弹窗也已在波次 10 收敛到 UiModal 外壳（原全局 `modal-mask` / `modal-card` 样式随之删除）。

页面需要展示调试辅助信息时，应使用 `UiState` 的 `development` variant 显式标记，而不是把它伪装成普通业务内容。正式 router 不注册独立开发入口。

全局样式与令牌约定由 `frontend/src/styles/tokens.test.js` 约束：它锁定规范批准的令牌值、对比度阈值、`var()` 引用与 hex fallback、页面级 `data-theme` 覆盖、z-index 语义令牌和 reduced-motion 守卫；波次 10 起它还承担全站遗留清零的全量守卫——原语内部类视图登记（仅管理后台四条）、非零字距登记（仅管理后台 eyebrow）与源代码手写 `<svg>` 零允许；新增全局样式时不要绕过这些 guardrail。
...
固定视觉矩阵位于 `tests/playwright-single/tests/08-visual.spec.ts`。它复用 single
套件的认证、路由和页面/API 错误审计 fixture，以 18 个页面状态、22 张 PNG 固定
当前 compact 桌面 UI；登录、Posts、PostDetail 和 Settings 同时覆盖明暗主题。
基线在 UI 迁移收尾后重建，只通过 `--update-snapshots` 有意更新并审查 diff。
