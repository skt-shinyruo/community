...
- `backend/community-im`：独立 IM 子系统，包含：
  ...
  - `im-session-ticket`：gateway 与 realtime 共用的短期 session ticket 编解码契约；gateway 签发 ticket，realtime 校验 ticket，im-core 不解析 ticket。
...
- `deploy/`：infra / single / cluster 三套本地 Stack、三个业务 schema 的空库定义和按 Stack 选择的 observability overlay。
