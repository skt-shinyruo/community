...
`npm run typecheck` 同时运行全量 Vue SFC/模板检查（`vue-tsc -p jsconfig.json`），以及
`tsc -p jsconfig.checked.json` 对 `src/**/*.js` 与 `vite.config.js` 的全量 `checkJs` 检查
（include 为 `src/**/*.js`、`src/**/*.d.ts`，`exclude` 必须保持为空；`.vue` 模块由
`src/jsconfig-shims.d.ts` 声明为松散类型，模板检查归属 `check:vue`）。存量类型债已清零：
任何 JS 文件出现新类型错误都应即修复，不允许把文件重新加回 `exclude`。
...
- `styles/*.test.js`：设计令牌与全局样式静态守卫（令牌值、对比度、`var()` 引用、`data-theme` 覆盖、z-index、reduced-motion、原语内部类视图登记、非零字距登记、手写 `<svg>` 零允许）。
- `components/ui/loading-states.test.js`：加载态静态守卫（裸「加载中」零允许、UiState 限定 empty / error / development、加载可访问名称统一由 UiSkeleton 提供）。
