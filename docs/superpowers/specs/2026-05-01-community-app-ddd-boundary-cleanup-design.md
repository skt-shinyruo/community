...
**Date:** 2026-05-01
**Status:** Implemented for Phase One through Phase Four
**Owner:** Codex
...
- `content.application.port` 与 `content.application.assembler` 包保持退役；
- `content.infrastructure.persistence` 下不再新增旧式 `*Service` 持久化实现；
- `market.domain.repository` 不暴露 mapper 风格 `select/insert/update` 方法；
- `market.application` 不再把 repository 字段命名为 `*Mapper`；
- `PostDetailView.vue` 与 `PostsView.vue` 拆分到小于复杂度阈值，并由 Vitest 守护；
- 后端全量 `community-app` 测试与前端全量 Vitest 测试通过。
...
- `5daae123` `docs: plan remaining ddd cleanup phases`
  - 将 Phase Two、Phase Three、Phase Four 展开为可执行计划，明确 guardrail、迁移清单、验证命令和提交边界。
- `a221025b` `refactor: converge content package shape`
  - 退役 `content.application.port` 与 `content.application.assembler`，把持久化契约迁入 `content.domain.repository`，把技术端口和 assembler 收敛到 application 根，并将 content 持久化实现重命名为 `MyBatis*Repository`。
- `6be13b88` `refactor: use domain names for market repositories`
  - 将 `market.domain.repository` 的 mapper 风格方法名改为领域语义方法名，application 字段从 `*Mapper` 改为 `*Repository`，并新增对应 ArchUnit 守护。
- `d8a9df09` `refactor: split post view workflows`
  - 拆分 `PostDetailView.vue` 与 `PostsView.vue` 的加载、草稿、交互、治理、发帖和 feed 逻辑，新增复杂度 Vitest 守护，并保持 UI/API 行为不变。

剩余阶段验收结果：

- `content.application.port`、`content.application.assembler`、`content.infrastructure.persistence.*Service` 已由 `DddLayeringArchTest` 守护；
- `market.domain.repository` mapper 风格方法名和 `market.application.*Mapper` 字段名已由 `DddLayeringArchTest` 守护；
- `PostDetailView.vue` 为 461 行，`PostsView.vue` 为 370 行，均低于 900 行复杂度阈值；
- `mvn -f backend/pom.xml -pl community-app -am test` 通过，`community-app` 724 tests passed；
- `npm test` 通过，54 test files / 188 tests passed；
- `npm run build` 通过。
...
本设计四个阶段已经全部落地：后端边界污染、`content` 包形状、`market` repository 语义和前端复杂度都有对应实现与可执行守护。后续新增业务代码应继续沿用这些 guardrail，避免重新引入 legacy package、mapper 语义或过大的视图文件。
