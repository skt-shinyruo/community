...
保留 Kafka topic `im.command.room-fanout-routed`，避免没有收益的命名变更。owner consumer 无条件执行 route + dispatch；Kafka dispatcher 是唯一 `RoomFanoutDispatcher`。它等待 broker ack，任一 target 失败时向 owner listener 抛错以触发原 persisted event 重试。已成功 target 通过稳定 `sourceEventId` 去重，因此部分成功后的整事件重试不会重复本地 fanout。
...
部署必须给每个并存 worker 唯一 inbox slot：single 为 `0`，三节点 cluster 为 `0/1/2`。本地 slot 缺失或越界时启动失败；service discovery 发现重复或缺失的远端 slot 时 routing fail closed。compose 静态测试在发布前保证已知拓扑 slot 唯一。topic partition 数继续为 64，并由部署脚本显式创建 topic 和 DLQ。
...
- owner 对多个 active worker 发送到正确 partition；partial failure 触发重试；相同 sourceEventId 在 target 只应用一次。
- single/cluster compose 测试锁定唯一 slots；HTTP endpoint 断言退休。
