...
async function reload() {
  ...
  pageError.value = ''
  try {
    const { data, hasNext: nextAvailable, page: loadedPage } = await listSellingMarketOrders({ page: 0, size: pageSize })
...
async function loadMore() {
  ...
  const scope = sessionScope.value
  try {
    const { data, hasNext: nextAvailable, page: loadedPage } = await listSellingMarketOrders({
      page: targetPage,
      size: pageSize
    })
    if (!isCurrentRequest(generation, scope)) return
    ...
    if (!isCurrentRequest(generation, scope)) return
    pageError.value = e?.message || '加载更多出售订单失败'
  } finally {
