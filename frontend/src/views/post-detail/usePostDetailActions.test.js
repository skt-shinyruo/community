...
describe('usePostDetailActions', () => {
  ...
  beforeEach(() => {
    ...
    vi.clearAllMocks()
    vi.mocked(setLike).mockResolvedValue({ data: { likeCount: 8, liked: true }, traceId: 'trace-like' })
    ...
    vi.mocked(bookmarkPost).mockResolvedValue({ traceId: 'trace-bookmark' })
    vi.mocked(unbookmarkPost).mockResolvedValue({ traceId: 'trace-unbookmark' })
    vi.mocked(blockUser).mockResolvedValue({ traceId: 'trace-block' })
    vi.mocked(unblockUser).mockResolvedValue({ traceId: 'trace-unblock' })
    vi.mocked(moderationTop).mockResolvedValue({ traceId: 'trace-top' })
    vi.mocked(moderationWonderful).mockResolvedValue({ traceId: 'trace-wonderful' })
    vi.mocked(moderationDelete).mockResolvedValue({ traceId: 'trace-delete' })
    vi.mocked(deletePostByAuthor).mockResolvedValue({ traceId: 'trace-author-delete' })
    vi.mocked(updatePost).mockResolvedValue({ traceId: 'trace-update-post' })
    vi.mocked(updateComment).mockResolvedValue({ traceId: 'trace-update-comment' })
  })
  ...
  it('keeps stale and failed requests from mutating the active page', async () => {
    ...
    vi.mocked(setLike).mockRejectedValueOnce(new Error('like unavailable'))
    await current.model.toggleLike()
    expect(current.error.value).toBe('like unavailable')

    vi.mocked(updatePost).mockRejectedValueOnce(new Error('save unavailable'))
