...
Directory: `helloagents/plan/202601291027_backend_arch_5issues_solution/`
...
- [√] 5.3 为 search 切换补齐契约/集成测试（至少覆盖分页/排序/高亮差异的约束），files: `search-service/src/test/java/com/nowcoder/community/search/api/SearchControllerTest.java`, `helloagents/wiki/modules/search.md`, verify why.md#requirement-r5-环境差异与-ssot-修复
...
- [√] 12.3 文档与 runbook：补齐事件演进与 unknown handling 约定，files: `docs/SYSTEM_DESIGN.md`, `helloagents/wiki/modules/common.md`, verify why.md#requirement-r12-事件版本治理一致化（envelope-version--unknown-handling）
...
- [√] 17.3 把“关键 fail-closed 默认值”从“仅 prod 文件存在”升级为“可验证兜底 + 启动期校验”（与 R14 联动），files: `gateway/src/main/resources/application-prod.yml`, `helloagents/wiki/runbooks/deploy-prod.md`, verify why.md#scenario-关键-fail-closed-默认值不能依赖“只在-prod-profile-才存在的文件”
...
- [√] 18.2 gateway：补齐“旁路禁止”的部署验收项与说明（只暴露 gateway/ingress），files: `deploy/docker-compose.ports.yml`, `helloagents/wiki/runbooks/security.md`, verify why.md#scenario-auth-service-被旁路访问时仍不降低安全性
- [√] 18.3 明确 SameSite/Secure/allowlist 的组合策略并写入 SSOT（含跨站方案），files: `helloagents/wiki/runbooks/cookie-and-csrf.md`, `docs/SYSTEM_DESIGN.md`, verify why.md#scenario-samesiteoriging-策略与前端部署形态一致
...
- [√] 20.3 gateway：补齐“common 自动装配生效”的启动验证（smoke），files: `gateway/src/test/java/com/nowcoder/community/gateway/StartupSmokeTest.java`, `helloagents/wiki/modules/gateway.md`, verify why.md#scenario-common-的-cross-cutting-能力在所有服务中一致生效
...
- [√] 28.5 deploy/docs：固化 OPS 操作 runbook（默认关闭→临时开启→执行→回滚→关闭）与可选“双人确认”流程，files: `helloagents/wiki/runbooks/internal-ops.md`, `docs/SECURITY.md`, verify why.md#scenario-reindex-必须单飞且可审计
