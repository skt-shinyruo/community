...
说明：对外运维入口统一由 `gateway` 暴露 `/api/ops/**`，并通过 Dubbo RPC 调用对应服务；服务端不再提供 HTTP `/internal/**` 运维入口（避免 internal HTTP 与 RPC 并存导致长期“半迁移”治理债务）。
...
- `POST /api/ops/content/likes/backfill?entityType=&maxItems=&batchSize=`：回填 Redis 点赞投影（content-service，高成本；默认关闭，需显式开启 `content.like.backfill.endpoint-enabled=true`）
...
- **gateway（ops handlers）**：
  - `gateway/src/main/java/com/nowcoder/community/gateway/ops/GatewayOpsController.java`
  - `gateway/src/main/java/com/nowcoder/community/gateway/ops/GatewayLegacyController.java`
