...
- `infra-security-starter/`：
  ...
  - 统一 JWT authorities converter（跨服务 SSOT）：`infra-security-starter/src/main/java/com/nowcoder/community/infra/security/jwt/AuthoritiesConverterFactory.java`
...
- `infra-dubbo-starter/`：
  - Dubbo traceId 透传（attachments）：consumer 优先使用 invocation attachment 作为 traceId SSOT；provider 注入 TraceContext/MDC 并 finally 清理
...
- `infra-outbox/`：
  ...
  - 应急直发开关（默认关闭）：`events.outbox.direct-send-enabled=false`（仅用于 outbox disabled 的应急止血，常态保持 outbox-only）
