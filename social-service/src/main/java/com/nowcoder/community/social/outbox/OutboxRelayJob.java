...
@Component
@ConditionalOnProperty(name = "social.events.outbox.enabled", havingValue = "true")
public class OutboxRelayJob {
    ...
    public OutboxRelayJob(
            ...
            SocialOutboxProperties properties,
        ...
        if (meterRegistry != null) {
            meterRegistry.gauge("social_outbox_backlog", Tags.of("status", "NEW"), newCountGauge);
            meterRegistry.gauge("social_outbox_backlog", Tags.of("status", "RETRY"), retryCountGauge);
            meterRegistry.gauge("social_outbox_backlog", Tags.of("status", "SENDING"), sendingCountGauge);
            meterRegistry.gauge("social_outbox_backlog", Tags.of("status", "FAILED"), failedCountGauge);
        }
    }

    @Scheduled(fixedDelayString = "${social.events.outbox.relay-interval-ms:5000}")
    public void relay() {
