...
test.describe.serial('market product flow @regression', () => {
  test('seller publishes, buyer orders, and seller manages inventory @regression', async ({ page }) => {
    ...
    await test.step('seller manages the order and inventory', async () => {
      ...
      const listingRow = page.locator('[data-test="my-listing-row"]').filter({ hasText: data.virtualListingTitle })
      ...
      // 失效库存是高风险写操作：先经确认弹窗复述内容，确认后才真正失效。
      // 首条预存库存是否仍处可售取决于托管处理时序，失效入口定位到本次追加的库存行。
      const inventoryRow = page.getByRole('row').filter({ hasText: data.virtualListingExtraInventory })
      await inventoryRow.getByRole('button', { name: '失效', exact: true }).click()
      const invalidateDialog = page.getByRole('dialog', { name: '失效库存内容' })
      await expect(invalidateDialog).toBeVisible()
      await expect(invalidateDialog.getByText(data.virtualListingExtraInventory)).toBeVisible()
      await invalidateDialog.getByRole('button', { name: '确认失效' }).click()
